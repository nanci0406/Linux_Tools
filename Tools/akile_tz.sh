...
EOF
  ...
  echo "AkileCloud Monitor 面板端已成功安装并启动！"
...
EOF
  ...
  echo "AkileCloud Monitor 监控端已成功安装并启动！"

  # 拉取配置文件
  wget -O /etc/ak_monitor/config.json https://raw.githubusercontent.com/akile-network/akile_monitor/refs/heads/main/client.json

  # 提示用户修改配置
  read -p "请输入面板端 auth_secret: " auth_secret
  sed -i "s/\"auth_secret\": \"面板端设置的key\"/\"auth_secret\": \"$auth_secret\"/g" /etc/ak_monitor/config.json

  read -p "请输入面板端的 IP 地址: " panel_ip
  sed -i "s/\"url\": \"ws://面板端ip:3000/monitor\"/\"url\": \"ws://$panel_ip:3000/monitor\"/g" /etc/ak_monitor/config.json

  # 自动识别网卡名称
  net_name=$(ip link show | grep -E 'eth|enp' | awk '{print $2}' | tr -d ':')
  sed -i "s/\"net_name\": \"自动识别主网卡名称并填入，如eth0或enp0s3等\"/\"net_name\": \"$net_name\"/g" /etc/ak_monitor/config.json

  # 让用户输入节点名称
  read -p "请输入节点名称: " node_name
  sed -i "s/\"name\": \"节点名称\"/\"name\": \"$node_name\"/g" /etc/ak_monitor/config.json

  echo "配置文件已更新，请检查 /etc/ak_monitor/config.json 是否符合要求。正在重启监控端"
